...
describe('should return pagination options', () => {
	test('empty string', async () => {
		expect(parsePaginationOptions(qs.parse(''), { defaultPageSize: 10 })).toStrictEqual(Pagination.forward(10))
	})
	test.each([
		{
			queryString: 'first=2',
		...
		expect(parsePaginationOptions(qs.parse(queryString), { defaultPageSize: 10 })).toStrictEqual(pagination)
	})
})
...
describe('should ignore parameters without values', () => {
	test.each([
		...
		expect(parsePaginationOptions(qs.parse(input), { defaultPageSize: 10 })).toStrictEqual(Pagination.forward(10))
	})
})
