...
describe('return sort options', () => {
	test.each([
		...
		expect(parseSorts(qs.parse(queryString))).toEqual(params)
	})
})

test('should handle patch with nested properties', () => {
	expect(parseSorts(qs.parse(`sort=desc(user.fullName)`))).toEqual([{ path: 'user.fullName', direction: 'desc' }])
})

test('should return many sort options', () => {
	expect(parseSorts(qs.parse('sort=asc(createdAt)&sort=desc(updatedAt)'))).toEqual([
...
test('should take first when sort path is used twice', () => {
	expect(parseSorts(qs.parse('sort=asc(createdAt)&sort=desc(createdAt)'))).toEqual([
		{ path: 'createdAt', direction: 'asc' }
	])
})
