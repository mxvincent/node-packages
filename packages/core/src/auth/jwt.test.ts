...
const getSymmetricKeyTools = (
	...
	switch (algorithm) {
		...
		case JWTAlgorithm.HS512:
			return {
				sign(payload, options) {
					return sign(payload, SECRET, { ...defaultSignOptions, ...options })
				},
				verify(token) {
					return verifyToken(token, SECRET, defaultTokenOptions)
				}
			}
		...
		case JWTAlgorithm.RS512:
			return {
				sign(payload, options) {
					return sign(payload, RSA_KEY_PAIR.privateKey, { ...defaultSignOptions, ...options })
				},
				verify(token) {
					return verifyToken(token, RSA_KEY_PAIR.publicKey, defaultTokenOptions)
				}
			}
		default:
