...
describe('set default sort', () => {
	test('should set map entry', () => {
		setDefaultSort(Author, [Sort.descending('createdAt')])
		expect(defaultSort.get(Author)).toEqual([Sort.descending('createdAt')])
	})
})

describe('get default sort', () => {
	test('should return custom sort options', () => {
		defaultSort.set(Author, [Sort.descending('createdAt')])
		expect(getDefaultSort(Author)).toEqual([Sort.descending('createdAt')])
	})
...
describe('appendPrimaryKeySorts()', () => {
	...
	test('should append primary key sort', async () => {
		const sorts: Sort<KeyOf<Pet>>[] = []
		expect(mergePrimaryKeySorts(sorts, ['id'])).toStrictEqual([Sort.ascending('id')])
	})

	test('should not override primary key sort', async () => {
		const sorts: Sort<KeyOf<Pet>>[] = [Sort.ascending('id')]
		expect(mergePrimaryKeySorts(sorts, ['id'])).toStrictEqual([Sort.ascending('id')])
	})
