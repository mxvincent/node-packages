...
import { base64Decode, base64Encode, sha1 } from '@mxvincent/utils'
...
export class Pager<T extends ObjectLiteral> extends Sorter<T> {
	...
	async getPage(pagination: Pagination): Promise<Page<T>> {
		...
		// Apply sorts
		this.applySorts(query, pagination.type === 'backward')
		...
		// Get page nodes from database
		const getPage = async () => {
			const getPageQuery = query.clone()
			if (pagination.cursor) {
				this.applyCursorConstraint(getPageQuery, pagination.type === 'forward' ? 'after' : 'before', pagination.cursor)
			}
			getPageQuery.limit(pagination.first)
			const data = await getPageQuery.getMany()
			return pagination.type === 'forward' ? data : data.reverse()
		}

		const data = await getPage()

		// Generate page info
		const { endCursor, startCursor } = await this.getPageCursors(data)
		// const [hasPrevPage, hasNextPage] = await Promise.all([
		// 	startCursor ? await hasItem('before', startCursor) : Promise.resolve(false),
		// 	endCursor ? await hasItem('after', endCursor) : Promise.resolve(false)
		// ])
		if (startCursor) {
	 ...
	 *
	 * with multiple parameter we need to apply many where conditions
	 *
	...
	private applyCursorConstraint(query: SelectQueryBuilder<T>, position: CursorRelativePosition, cursor: string) {
		...
		const whereGreaterOrLower = (sort: Sort) => {
			const operator = xor(position === 'after', sort.direction === SortDirection.ascending) ? '<' : '>'
			return `${getAliasedPath(sort.path, query.alias)} ${operator} :${sha1(sort.path)}`
		}
		const applyNextPairContraint = (parentQueryBuilder: WhereExpressionBuilder, sorts: [Sort, Sort]) => {
