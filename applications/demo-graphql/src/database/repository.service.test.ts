...
import { Pagination, QueryParameters } from '@mxvincent/query-params'
...
describe('Repository.getGQLConnection()', () => {
	...
	test('should return 10 first users', async () => {
		...
		const connection = await repository.getConnection(new QueryParameters({ pagination: Pagination.forward(10) }))
...
describe('Repository.getPage()', () => {
	...
	test('should return 10 first users', async () => {
		...
		const page = await repository.getPage(new QueryParameters({ pagination: Pagination.forward(10) }))
